...
function LowStock() {
  ...
  useEffect(() => {
    const fetchLowStock = async () => {
      try {
        ...
        setErrorMsg("❌ Failed to fetch low stock items.");
        console.error(error);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-lime-200 via-emerald-100 to-cyan-100 p-10 relative">
      ...
      <h1 className="text-3xl font-bold text-red-700 mb-6">⚠️ Low Stock Alerts</h1>
      ...
      {lowStockItems.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {lowStockItems.map((item) => (
            <div
              key={item._id}
              className="rounded-xl border border-red-300 bg-white/80 backdrop-blur-sm p-5 shadow-[0_8px_16px_rgba(0,0,0,0.05)] transition hover:shadow-md"
            >
              ...
              <div className="text-sm text-gray-700 space-y-1">
                ...
                <p className="text-gray-500">
                  ...
                  <strong>Expiry:</strong> {new Date(item.expiryDate).toLocaleDateString("en-GB")}
                </p>
              </div>
            </div>
          ))}
